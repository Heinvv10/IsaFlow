...
async function handler(req: CompanyApiRequest, res: NextApiResponse) {
  ...
  const companyRows = await sql`SELECT name FROM companies WHERE id = ${req.companyId} LIMIT 1` as Row[];
  const companyName = String((companyRows[0] as Record<string, unknown>)?.name || 'Unknown Company');
  ...
  const debtor: DebtorProfile = { name: String(c.name), contactPerson: String(c.contact_person || c.name), email: String(c.email || ''), outstandingAmount: outstanding, oldestInvoiceDays: oldestDue, invoiceCount, companyName };
